...
func (m *Manager) spawnLocked(model chatx.Model) (*exec.Cmd, *State, chan error, error) {
	...
	// -c 8192：过滤 prompt ~1-2K token 留足余量，内存可控。
	// --chat-template-kwargs 统一压 qwen3 混合思考模型的 thinking；非思考版
	// （4B-Instruct）模板不消费该键，llama.cpp 只把模板消费的键传入，带之无害。
	args := []string{
		"-m", modelPath,
...
// 后状态文件不落盘，下轮 Reconcile（want 标记在失败时保留，chat 调用失败也会重写
