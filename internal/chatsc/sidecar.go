// Package chatsc 管理内置 chat 推理 sidecar（llama-server）：
...
// State 是 chat-sidecar.json 的内容：sidecar 发现与身份。
type State struct {
	PID       int       `json:"pid"`
	Port      int       `json:"port"`
...
func statePath() string { return filepath.Join(registry.Home(), "chat-sidecar.json") }
func wantPath() string  { return filepath.Join(registry.Home(), "chat-sidecar.want") }
func logPath() string   { return filepath.Join(registry.Home(), "chat-sidecar.log") }
...
// Touch 更新 last_used（chat 调用成功后由客户端包装层调用；失败静默）。
func Touch() {
	st := LoadState()
	if st == nil {
		return
