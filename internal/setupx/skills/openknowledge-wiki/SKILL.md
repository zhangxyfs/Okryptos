...
4. **写条目**（规范见下）：第一条固定是"架构总览"，然后**按顶层目录/主题域聚合出条目**（十来个为宜，纯工具包不单独成条），最后一条"演进历程"记里程碑。
...
3. **演进历程追加前查阈值**：正文 > 6000 字或里程碑 > 25 个（先到先触发）时，先把最老一段切到归档条目 `演进历程-归档（<版本段>）`（如 `演进历程-归档（v1.0~v2.15）`，tags `wiki,历史`），再重写主条目——主条目只保留近期里程碑 + 一行早期简述 + 指向归档条目的指针。里程碑跨目录是常态，主线保持时间轴，不按目录拆。
...
- 归档条目命名 `演进历程-归档（<版本段>）`，tags `wiki,历史`；同为 wiki 条目，可检索可读，不丢信息。
...
- 存量 wiki 条目（tags 含 `wiki` 且 type=reference）无需改写，自动归为主题域章节；架构总览重写为目录地图前逐段核对旧正文——独有细节先搬进对应主题域条目再重写，不允许只删不搬。
