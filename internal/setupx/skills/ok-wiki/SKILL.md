...
4. **写条目**（规范见下）：第一条固定是"架构总览"，然后**按顶层目录/主题域聚合出条目**（十来个为宜，纯工具包不单独成条），最后是"演进历程"——它同样只写**索引**（一个版本段一行 + 指针），里程碑正文拆成版本段子条目 `演进历程（<版本段> <主题>）`（tags `wiki,历史`，GUI 树挂在演进历程节点下）。
   - "架构总览"只写**目录地图**：每个主题域一行（职责一句话 + 指向对应主题域条目的指针），总览本身不承载易变细节——细节下沉到主题域条目。
5. **推进游标**：
   ```bash
   "{{EXE}}" wiki mark
   ```
...
1. 取游标范围内的变更：
   ...
   ```
2. 只消化这段变更：按 `git diff --stat` 的路径把变更**路由到所属主题域条目**，用同名 `add --force` 重写覆盖；新主题新增条目。
3. **演进历程维护**：新里程碑追加进最近一个版本段子条目（同名 `add --force` 重写），主条目（索引）同步刷新对应行；某段超过 ~15 个里程碑或主题跃迁明显时，把它细分成两段（新段 `add`、旧段 `--force` 截断、索引行随之拆分）。段条目**不置 archived**——它们常驻"演进历程"节点下、可检索、进 INDEX Wiki 目录；演进历程靠"索引瘦身 + 段拆分"控制体量，不走归档。
4. `"{{EXE}}" wiki mark` 推进游标，汇报变更摘要。
...
- 主题域条目可带**"沿革"小节**：本模块自己的版本节点，三五条即可——目录级历史随条目走，不进"演进历程"主线。
- 演进历程版本段条目命名 `演进历程（<版本段> <主题>）`，tags `wiki,历史`——GUI 树按"历史"tag 挂到演进历程节点下，按段标题首个版本号数值升序；不设 archived，可检索可读，不丢信息。
- 内容写"为什么"和"怎么协作"，不抄代码；关键文件用 `path:行号` 引用。

## 存量 wiki 迁移纪律（首次按新规跑时执行）

- **备份先行**：迁移前必须导出备份（`"{{EXE}}" export` 或 GUI 导出 zip）——knowledge 数据不在 git，备份是唯一回滚手段。
- **全部走 `add --force` 正式写路径**（fsx 锁 + 索引/向量/INDEX 同步），不直接手改 `knowledge/` 目录文件。
- **幂等可重入**：版本段条目已存在则 `--force` 覆盖更新（`add --force` 会继承盘上原条目的 draft/archived/created，不会静默转正）；主条目已是索引形态则跳过，重复执行不产生重复条目。
- 存量 wiki 条目（tags 含 `wiki` 且 type=reference）无需改写，自动归为主题域章节；架构总览重写为目录地图、演进历程重写为索引前，都要逐段核对旧正文——独有细节先搬进对应子条目再重写，不允许只删不搬。
