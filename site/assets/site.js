...
(function () {
  ...
  var EN = {
    ...
    'i.src': 'Source',
    'i.hint': 'Auto-detects your OS · <a href="https://github.com/zhangxyfs/Okryptos/releases/download/v2.26.4/OkryptosSetup-2.26.4.exe">Windows</a> / <a href="https://github.com/zhangxyfs/Okryptos/releases/download/v2.26.4/openknowledge_2.26.4_amd64.deb">Linux .deb</a> / <a href="https://github.com/zhangxyfs/Okryptos/releases/download/v2.26.4/openknowledge_2.26.4_linux_amd64.tar.gz">Linux .tar.gz</a> · <a href="https://github.com/zhangxyfs/Okryptos/releases" target="_blank" rel="noopener">All releases</a>',
    'i.feat.h': 'Features',
    ...
    'i.shots.sub': 'Six GUI tabs in action + knowledge capture in a real session',
    'i.s1c': 'Manage tab: project entry tree, README preview and the chat terminal panel',
    'i.s2c': 'Graph tab: entry relation graph — center-out radial layout with semantic edges',
    'i.s3c': 'Guide tab: one-click hooks and skills setup for ten agents',
    'i.s4c': 'Server tab: multi-device sync member view, project bindings and credential management',
    'i.s5c': 'Settings tab: embedding / LLM model profiles and global switches',
    'i.s6c': 'Logs tab: injection, filtering and janitor logs in real time',
    'i.s7c': 'In a real session: after finishing a release, the AI captures wiki entries on its own (propose mode)',
    'i.inst.h': 'Install',
    ...
    'c.cta': 'Download latest v2.26.4',
    'c.latest': 'Latest',
    'c.2264.f1': 'The config center and dock now come back on their own after an upgrade: previously a one-click upgrade force-closed the config center and the token meter, and only the daemon was restarted when setup finished — both were gone until reopened by hand; the installer now snapshots which companions are running before closing them and restores them afterwards, identically for interactive and silent upgrades, without double-opening the config center in interactive fast upgrades',
    'c.2264.f2': 'Fixed the config center occasionally not reopening after an upgrade: the daemon-readiness budget grows from 3s to 30s — a freshly upgraded binary often needs more than 3s to cold-start (antivirus scan, component init), and the old budget gave up and exited',
    'c.2264.n1': 'An upgrade-experience fix release with no data or config format changes; when upgrading from an older version to this one, setup is still performed by the old installer — the restore behavior takes effect from this version onward',
    'c.2263.i1': 'Zero-click upgrades: when the installer version is not older than the installed one, setup follows the previously chosen language, skips every wizard page, force-closes processes holding the files, and opens the config center as soon as it finishes; first-time and downgrade installs keep the full flow',
    'c.2263.i2': 'The collapsed dock&#39;s expand trigger is now configurable: a new "expand" setting offers hover (default, expand on entry) or click (left-click an item to expand); once expanded, hover detail cards and auto-retract behave the same in both modes',
    'c.2263.n1': 'A small polish release with no data or config format changes — existing metric mappings, dock edge and other settings are preserved',
    'c.2262.f1': 'The dock can now snap to the top edge or just above the taskbar: the edge setting extends from left/right to all four edges, with top/bottom rendering as a centered horizontal strip; collapsed state transforms into mini chips showing only name and reading, expand/collapse is an item-by-item morph animation with auto-retract on mouse leave, and switching visual forms never flickers; detail card, right-click menu and settings panel auto-adjust their popup direction; wave form always flows left-to-right when horizontal',
    'c.2262.i1': 'The right-click "Models" menu now groups by real provider: a model used by several agents is grouped under one provider (e.g. k3 and k3-256k under Moonshot) instead of being scattered across per-tool columns; duplicate model names are suffixed with the source tool (e.g. k3 · kimi-code)',
    'c.2262.n1': 'Upgrading keeps your existing metric mappings and dock edge; the classic left/right vertical dock behaves pixel-identically',
    'c.2261.f1': 'OkMeter now collects from nine agents: beyond Kimi Code, eight new adapters for Claude Code, Codex, Qwen Code, ZCode, WorkBuddy, Reasonix, OpenHanako and DeepSeek Harness — token usage from each tool&#39;s local session logs flows into the stats automatically, grouped by tool in the right-click "Models" menu; every directory is change-watched so fresh data shows up instantly',
    'c.2261.i1': 'Wave form renders a full-width flat line at startup instead of waiting for history to accumulate; flow direction mirrors with the dock edge — right-to-left on the left edge, left-to-right on the right edge, so "now" always faces the screen center',
    'c.2261.i2': 'Hover detail card no longer stacks on top of an open right-click menu; the settings panel metric-mapping dropdown now uses the same cascading multi-level menu as right-click, fixing the menu drifting left on its own and being occluded by the settings panel',
    'c.2261.n1': 'Token scopes are normalized across agents (cache read / cache creation split out); OpenCode is unsupported for now since its usage lives in sqlite',
    'c.2261.n2': 'DeepSeek Harness session logs are compressed, so stats read its session projection cache instead, with the model name approximated from the default config',
    'c.2260.f1': 'Chinese name "归藏" (Guīzàng): the GUI header brand area and browser title switch between 归藏 / Okryptos with the language, and the site hero plus bilingual READMEs gain the name&#39;s origin and the tagline "a latent knowledge base for AI-agent hooks"',
    'c.2260.f2': 'OkMeter token dock (Windows): a persistent token-usage meter docked to the screen edge — collapsed to a sliver, slides out on hover; total (session/today/week/all-time) and per-model scopes, switched instantly from a cascading right-click menu; hover detail card; six forms × four materials, with true background refraction in liquid glass; back-panel settings; enabled by default via the "token stats tool" switch in the config center, launchable from the tray menu',
    'c.2260.f3': 'Two local LLM profiles: an ollama profile (key-free, talks to your local ollama) and a builtin managed profile (one-click model download in the GUI with progress — qwen3 1.7B recommended / 0.6B smallest / 4B non-thinking most stable) that runs fully local afterwards, so intent filtering costs zero API calls',
    'c.2260.f4': 'LLM relevance post-filter for retrieval: before injection, candidate entries are judged by the LLM — it can only remove, never add; unconfigured, timed-out or unparseable calls keep all original results, so existing retrieval is never degraded',
    'c.2260.f5': 'Dual "in use" slots for intent recognition: the LLM config gains a dedicated intent slot, so filtering calls and general calls can use different models',
    'c.2260.i1': 'Keyword-channel coverage admission: short colloquial queries (e.g. "还是黑底的") no longer mis-inject unrelated entries through stop-word hits — matched term count must pass a ratio gate, and pure stop-word queries skip the keyword channel entirely',
    'c.2260.i2': 'GUI model config dialog: builtin profile shows model size and resource estimates, intent notes wrap per line, and the dual-slot badges render as two rows',
    'c.2260.n1': 'The OkMeter dock ships with the Windows installer only (portable exe works too, but without single-instance guarding)',
    'c.2260.n2': 'Builtin managed models download locally (640MB–2.5GB) and use roughly 1–4GB RAM at runtime — pick the ollama or remote API profile if resources are tight',
