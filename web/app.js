...
const I18N = {
  zh: {
    ...
    srvOrgs:"组织", srvOrgsDesc:"团队功能预备：当前可建组织、维护成员、预建团队仓；条目晋升与团队仓内容同步（PR 人审 / 服务端 wiki 管线）尚未上线，组织暂不产生共享知识。点成员 chip 加/减成员。", srvOrgName:"组织名称", srvDescOpt:"描述（可选）",
    ...
    srvDeploy:"部署指引（还没有服务器？展开）", srvUserCreated:"用户已创建", srvCopyHint:"初始密码仅本次显示，请立即复制发给用户。",
  ...
  en: {
    ...
    srvOrgs:"Organizations", srvOrgsDesc:"Team groundwork: you can create orgs, manage members and pre-provision team repos. Entry promotion and team-repo content sync (PR review / server wiki pipeline) are not live yet — orgs share no knowledge for now. Toggle member chips to add/remove members.", srvOrgName:"Org name", srvDescOpt:"Description (optional)",
    ...
    srvDeploy:"Deployment guide (no server yet? expand)", srvUserCreated:"User created", srvCopyHint:"The initial password is shown only once — copy it now.",
...
async function llmApply(){
  ...
  for(const p of draft.profiles){
    await api("/api/llm/profile", { method:"POST", body:{
      ...
      filter:!!p.filter, activate:false } });   // api_key 空/掩码 = 同名保留旧 key（后端收口）
  }
  const keep = {}; draft.profiles.forEach(p=>{ keep[p.name]=true; });
  for(const sp of old.profiles||[]){
    if(!keep[sp.name]){
  ...
  if(draft.active !== serverActive){
    await api("/api/llm/active", { method:"POST", body:{ name:draft.active, slot:"general" } });   // 空串 = 停用
  }
...
function renderLlmModal(){
  ...
  llmDraft.profiles.forEach((p,idx)=>{
    ...
    del.onclick = ()=>{
      ...
      if(llmDraft.activeFilter===p.name) llmDraft.activeFilter = "";
