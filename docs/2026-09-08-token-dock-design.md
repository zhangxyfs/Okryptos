...
- 每个项可单独配置显示指标（右键该项 → 三级级联菜单选择）：
  - **一级**：总量 ▸ / 模型 ▸ / 设置 / 换边 / 退出（不再有"默认·按最近使用"项）；
  - **总量 ▸ 二级**：当前会话（最近活跃 session 的累计）/ 今日用量（本机自然日）/ 本周用量 / 全部累计；
  - **模型 ▸ 二级**：按 modelId 首段聚合的厂商分组（kimi-code / zhipuai-coding-plan / deepseek 等，按最近用排序）；
  - **厂商 ▸ 三级**：该厂商下已观测到的具体模型；
  - 悬停父项即展开子列（Windows 菜单同款），子列向屏内侧逐级展开。
- 项上数字用紧凑格式（`9.9K` / `128.6K` / `2.1M`），精确值留给悬停详情。

### 3.4 悬停交互与详情

- **悬停反馈（原型 .hot/.dim 语义）**：悬停项按形态放大（弧线 1.34 / 胶囊 1.08 / 罗盘 1.22 / 电平柱·波形流 1.07 / 辉光 1.14），**其余项降不透明度**（球体 .6，块状形态 .55；用户裁决：任何形态均不回缩尺寸）；块状项悬停**不画 accent 描边**（用户裁决），球体弧线相邻项沿弧线让位。动画 150~200ms 缓动。
...
右键任意球 → 三级级联菜单（见上）；右键弧线/空白 → 设置 / 换边 / 退出。
...
- **比例法定尺寸**：一切几何（项宽高/间距/角半径/内部 padding/字号/卡片尺寸）= 原型值 × `uiScale(屏高)`，基准画布 = 原型设计分辨率 1920×1080（钳 [0.7, 1.6] 防极端屏）；禁止再写死与原型一致的魔法像素——新增形态/控件一律走比例。
