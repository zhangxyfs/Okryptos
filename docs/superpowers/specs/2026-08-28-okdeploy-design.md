...
- Docker 分两档探测：CLI 存在（`command -v docker`）≠ 当前用户能连 daemon（NAS 常见权限问题）；失败时真实 stderr 摘要带回前端。**没有 Docker → 红色提示 + 安装指引，终止流程**（v1 不代装 Docker）。
- **sudo 回退（2026-08-30 增补，已实施）**：daemon 直连被拒时，探测自动用登录密码试 `sudo -kS`（密码仅存内存）；验证通过则会话切到 sudo 包装（`WrapSudo`），后续部署/管理命令全部带 sudo；私钥登录或 sudo 密码不同时，探测页给显式密码输入（`POST /api/enable-sudo` 验证后启用）。
